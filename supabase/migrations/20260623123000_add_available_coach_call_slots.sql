-- Add client-safe available coach call slots and enforce slot availability on booking requests.
-- V1 booking behaviour:
-- - 30-minute slots only.
-- - Clients only see generated available slots.
-- - Requested, accepted, and reschedule-pending bookings block the slot for other clients.
-- - Coach calendar blocks and blocked days also remove slots.
...
declare
  ...
  selected_window_days integer := 14;
begin
  select p.id
  ...
  select
    ...
    least(greatest(coalesce(p_days_ahead, s.booking_window_days, 14), 1), 90)
  ...
  if selected_window_days is null then
    selected_window_days := least(greatest(coalesce(p_days_ahead, 14), 1), 90);
  end if;

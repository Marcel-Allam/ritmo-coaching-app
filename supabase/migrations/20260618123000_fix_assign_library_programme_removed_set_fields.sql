-- Keep programme assignment compatible with the current set schema.
-- The assignment flow copies Library programmes into client-specific training tables.
...
begin
  ...
  for v_programme_workout in
    select lpw.workout_order, lw.id as library_workout_id, lw.name, lw.instructions
  ...
  loop
    insert into public.program_workouts (client_id, program_id, title, workout_order, scheduled_date, instructions, status, source_library_workout_id)
    values (p_client_id, v_new_program_id, v_programme_workout.name, v_programme_workout.workout_order, null, v_programme_workout.instructions, 'active', v_programme_workout.library_workout_id)
    ...
    loop
      ...
      for v_library_set in
        select id, set_order, target_reps, target_weight_kg, target_rpe, notes
        from public.library_workout_sets
        where library_workout_exercise_id = v_library_exercise.id
        order by set_order asc
      loop
        insert into public.program_sets (exercise_id, set_order, target_reps, target_weight_kg, target_rpe, notes, source_library_set_id)
        values (v_new_exercise_id, v_library_set.set_order, v_library_set.target_reps, v_library_set.target_weight_kg, v_library_set.target_rpe, v_library_set.notes, v_library_set.id);
      end loop;
    end loop;
  end loop;

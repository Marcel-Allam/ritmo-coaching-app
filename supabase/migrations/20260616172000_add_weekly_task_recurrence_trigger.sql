-- Create the next weekly client task only after the current one is completed.
-- This prevents overdue weekly tasks from generating a chain of future duplicates.
...
declare
  ...
  v_instructions text;
begin
  if new.submission_type not in ('training_availability', 'bodyweight') then
    return new;
  end if;
...
create trigger task_submissions_create_next_weekly_client_task
after insert on public.task_submissions
for each row
execute function public.create_next_weekly_client_task();

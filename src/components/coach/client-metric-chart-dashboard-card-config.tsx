...
const getDefaultConfigs = (clientId: string, exerciseNames: string[], hasBodyweightData: boolean): ChartConfig[] => {
  ...
  return [1, 2, 3].map((slot) => {
    if (slot === 1) {
      return { client_id: clientId, slot, source_type: 'exercise', exercise_name: bench, metric_key: 'estimated_1rm', title: null };
    }
    if (slot === 2 && hasBodyweightData) {
      return { client_id: clientId, slot, source_type: 'bodyweight', exercise_name: null, metric_key: 'bodyweight', title: null };
    }
...
const ChartCard = ({ config, points, isEditing, editableSlot, exerciseNames, hasBodyweightData, saving, onToggleEdit, onChange, onSave }: ChartCardProps) => {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      <div className="mb-4 flex items-start justify-between gap-3">
        ...
        <button type="button" onClick={onToggleEdit} className="rounded bg-black px-3 py-2 text-[10px] font-bold uppercase text-white hover:bg-gray-900">
      ...
      {isEditing && (
        <div className="mt-4 rounded-xl border-2 border-[#FA0201] bg-gray-50 p-4">
          <div className="mb-3">
            <p className="text-xs font-black uppercase text-[#000000]">Configure this chart</p>
            <p className="mt-1 text-[10px] font-bold uppercase text-gray-500">Title is automatic: source + y-axis.</p>
          </div>

          <div className="space-y-3">
            <label className="block">
              <span className="mb-1 block text-[10px] font-bold uppercase text-gray-500">Source</span>
              <select value={editableSlot.source_type} onChange={(event) => onChange({ source_type: event.target.value as SourceType })} className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-[#000000]">
                <option value="exercise">Exercise</option>
                <option value="bodyweight" disabled={!hasBodyweightData}>Bodyweight{hasBodyweightData ? '' : ' (no data)'}</option>
              </select>
            </label>

            {editableSlot.source_type === 'exercise' && (
              <>
                <label className="block">
                  <span className="mb-1 block text-[10px] font-bold uppercase text-gray-500">Exercise</span>
                  <select value={editableSlot.exercise_name} onChange={(event) => onChange({ exercise_name: event.target.value })} className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-[#000000]">
                ...
                <label className="block">
                  ...
                  <select value={editableSlot.metric_key} onChange={(event) => onChange({ metric_key: event.target.value as MetricKey })} className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-[#000000]">
            ...
            <div className="rounded-lg bg-white px-3 py-2">
              ...
              <p className="text-xs font-black uppercase text-[#000000]">{getGeneratedChartTitle({ source_type: editableSlot.source_type, exercise_name: editableSlot.exercise_name, metric_key: editableSlot.source_type === 'bodyweight' ? 'bodyweight' : editableSlot.metric_key })}</p>
            </div>

            <button type="button" onClick={onSave} disabled={saving} className="w-full rounded-lg bg-[#FA0201] px-4 py-2 text-xs font-bold uppercase text-white hover:bg-red-700 disabled:opacity-60">
...
export function ClientMetricChartDashboardCardConfig({ clientId }: { clientId: string }) {
  ...
  const loadDashboard = async () => {
    ...
    setEditableSlots(hydratedConfigs.map((config) => ({
      slot: config.slot,
      source_type: config.source_type,
      exercise_name: config.exercise_name || exerciseNames[0] || '',
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <p className="text-sm text-gray-700">Choose up to 3 client-specific performance charts. Configure each chart directly from its own card.</p>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        {configs.slice(0, 3).map((config) => {
          const editableSlot = editableSlots.find((slot) => slot.slot === config.slot) || { slot: config.slot, source_type: config.source_type, exercise_name: config.exercise_name || '', metric_key: config.metric_key };
          return (
            <ChartCard
              key={`${config.slot}-${config.metric_key}-${config.exercise_name || 'bodyweight'}`}
              config={config}
              points={getChartPoints(config)}
              isEditing={editingSlot === config.slot}
              editableSlot={editableSlot}
              exerciseNames={exerciseNames}
              hasBodyweightData={hasBodyweightData}
              saving={savingSlot === config.slot}
              onToggleEdit={() => setEditingSlot((current) => (current === config.slot ? null : config.slot))}
              onChange={(patch) => updateEditableSlot(config.slot, patch)}
              onSave={() => saveSlot(editableSlot)}
            />
          );
        })}
      </div>

...
const metricLabelMap: Record<MetricKey, { label: string; suffix: string }> = {
  estimated_1rm: { label: 'Estimated 1RM', suffix: 'kg' },
  top_weight: { label: 'Top load', suffix: 'kg' },
  volume: { label: 'Volume', suffix: 'kg' },
  top_reps: { label: 'Top-set reps', suffix: ' reps' },
  completed_sets: { label: 'Completed sets', suffix: ' sets' },
  avg_reps_per_set: { label: 'Average reps/set', suffix: ' reps' },
  bodyweight: { label: 'Bodyweight', suffix: 'kg' },
};
...
const getDefaultConfigs = (clientId: string, exerciseNames: string[], hasBodyweightData: boolean): ChartConfig[] => {
  ...
  if (squat && defaults.length < 3) {
    defaults.push({
      client_id: clientId,
      slot: defaults.length + 1,
      source_type: 'exercise',
      exercise_name: squat,
      metric_key: 'volume',
      title: `${squat} volume`,
  ...
  return [1, 2, 3].map((slot) => {
    ...
    return {
      ...
      slot,
      source_type: 'exercise',
      exercise_name: exerciseNames[0] || null,
      metric_key: 'estimated_1rm',
      title: exerciseNames[0] ? `${exerciseNames[0]} estimated 1RM` : `Metric ${slot}`,
    };
  });
};
        ...
        {coordinates.map((point, index) => (
          <g key={`${point.label}-${index}`} className="cursor-pointer" onClick={() => setSelectedIndex(index)}>
            ...
            {(index === 0 || index === coordinates.length - 1) && (
              <text x={point.x} y={height - 6} textAnchor="middle" className="fill-gray-500 text-[10px] font-semibold">
                {point.label}
              </text>
            )}
          </g>
...
const ChartCard = ({ config, points }: { config: ChartConfig; points: ChartPoint[] }) => {
  ...
  const title = cleanText(config.title) || (config.source_type === 'bodyweight' ? 'Bodyweight' : `${config.exercise_name} ${metric.label}`);

  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-black uppercase text-[#000000]">{title}</p>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        <div className="rounded-lg bg-gray-50 p-2">
          ...
          <p className="text-sm font-black text-[#000000]">{formatValue(latestValue, metric.suffix)}</p>
        </div>
        <div className="rounded-lg bg-gray-50 p-2">
          <p className="text-[10px] font-bold uppercase text-gray-500">1 session</p>
          <p className="text-sm font-black text-[#000000]">{formatPercent(sessionChange)}</p>
        </div>
        <div className="rounded-lg bg-gray-50 p-2">
          <p className="text-[10px] font-bold uppercase text-gray-500">4 weeks</p>
          <p className="text-sm font-black text-[#000000]">{formatPercent(fourWeekChange)}</p>
        </div>
      </div>
    </div>
  );
...
export function ClientMetricChartDashboard({ clientId }: { clientId: string }) {
  ...
  const loadDashboard = async () => {
    ...
    const [configResult, performedSetsResult, bodyweightResult] = await Promise.all([
      supabase
        .from('client_metric_chart_configs')
        .select('id, client_id, slot, source_type, exercise_name, metric_key, title')
  ...
  const saveSlot = async (slot: EditableSlot) => {
    ...
    const payload = {
      ...
      title: cleanText(slot.title) || null,
    };

    const supabase = createClient();
    const { error: saveError } = await supabase
      .from('client_metric_chart_configs')
      .upsert(payload, { onConflict: 'client_id,slot' });
  ...
  if (loading) {
    return <Card>Loading performance charts...</Card>;
  }

  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <p className="text-sm text-gray-700">Choose up to 3 client-specific performance charts for this profile. Each chart is built from completed workouts or bodyweight entries.</p>
        ...
        <div className="rounded-xl border-2 border-[#FA0201] bg-white p-4">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs font-semibold uppercase text-gray-500">Maximum 3 chart slots.</p>
            </div>
                ...
                <div className="space-y-3">
                  ...
                  {slot.source_type === 'exercise' && (
                    <>
                      ...
                      <label className="block">
                        <span className="mb-1 block text-[10px] font-bold uppercase text-gray-500">Metric</span>
                  ...
                  <label className="block">
                    <span className="mb-1 block text-[10px] font-bold uppercase text-gray-500">Optional title</span>
                    <input value={slot.title} onChange={(event) => updateEditableSlot(slot.slot, { title: event.target.value })} className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-[#000000]" placeholder="Example: Bench e1RM" />
                  </label>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        {configs.slice(0, 3).map((config) => (
          <ChartCard key={`${config.slot}-${config.metric_key}-${config.exercise_name || 'bodyweight'}`} config={config} points={getChartPoints(config)} />
        ))}
      </div>
    </div>
  );

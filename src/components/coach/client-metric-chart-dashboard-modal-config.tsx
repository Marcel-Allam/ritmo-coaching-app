...
const exerciseMetrics: Array<{ key: ExerciseMetricKey; label: string; titleLabel: string; suffix: string }> = [
  { key: 'estimated_1rm', label: 'Estimated 1RM', titleLabel: 'estimated 1RM', suffix: 'kg' },
  { key: 'top_weight', label: 'Top load', titleLabel: 'top load', suffix: 'kg' },
  { key: 'volume', label: 'Volume', titleLabel: 'volume', suffix: 'kg' },
  { key: 'top_reps', label: 'Top-set reps', titleLabel: 'top-set reps', suffix: ' reps' },
  { key: 'completed_sets', label: 'Completed sets', titleLabel: 'completed sets', suffix: ' sets' },
  { key: 'avg_reps_per_set', label: 'Average reps/set', titleLabel: 'average reps/set', suffix: ' reps' },
];
...
const MiniLineChart = ({ points, suffix }: { points: ChartPoint[]; suffix: string }) => {
  ...
  if (points.length === 0) {
    return <div className="flex h-36 items-center justify-center rounded-lg bg-gray-50"><p className="text-xs font-semibold uppercase text-gray-500">No data yet</p></div>;
  }
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full">
        ...
        {coordinates.map((point, index) => (
          <g key={`${point.label}-${index}`} className="cursor-pointer" onClick={() => setSelectedIndex(index)}>
            ...
            {(index === 0 || index === coordinates.length - 1) && <text x={point.x} y={height - 6} textAnchor="middle" className="fill-gray-500 text-[10px] font-semibold">{point.label}</text>}
          </g>
        ))}
      </svg>
    </div>
...
const ChartCard = ({ config, points, onConfigure }: { config: ChartConfig; points: ChartPoint[]; onConfigure: () => void }) => {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      <div className="mb-4 flex items-start justify-between gap-3">
        ...
        <button type="button" onClick={onConfigure} className="rounded bg-black px-3 py-2 text-[10px] font-bold uppercase text-white hover:bg-gray-900">Configure</button>
      </div>
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 py-6" role="dialog" aria-modal="true">
    <div className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-2xl">
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          <p className="text-lg font-black uppercase text-[#000000]">Configure chart</p>
          <p className="mt-1 text-xs font-semibold uppercase text-gray-500">Chart slot {slot.slot}. Title is automatic.</p>
        </div>
        <button type="button" onClick={onClose} className="rounded-lg bg-black px-3 py-2 text-xs font-bold uppercase text-white hover:bg-gray-900">Close</button>
      </div>

      <div className="space-y-4">
        <label className="block">
          <span className="mb-1 block text-xs font-bold uppercase text-gray-500">Source</span>
          <select value={slot.source_type} onChange={(event) => onChange({ source_type: event.target.value as SourceType })} className="w-full rounded-lg border border-gray-300 bg-white px-3 py-3 text-sm text-[#000000]">
        ...
        {slot.source_type === 'exercise' && (
          <>
            <label className="block">
              <span className="mb-1 block text-xs font-bold uppercase text-gray-500">Exercise</span>
              <select value={slot.exercise_name} onChange={(event) => onChange({ exercise_name: event.target.value })} className="w-full rounded-lg border border-gray-300 bg-white px-3 py-3 text-sm text-[#000000]">
            ...
            <label className="block">
              <span className="mb-1 block text-xs font-bold uppercase text-gray-500">Y-axis / metric</span>
              <select value={slot.metric_key} onChange={(event) => onChange({ metric_key: event.target.value as MetricKey })} className="w-full rounded-lg border border-gray-300 bg-white px-3 py-3 text-sm text-[#000000]">
                {exerciseMetrics.map((metricOption) => <option key={metricOption.key} value={metricOption.key}>{metricOption.label}</option>)}
              </select>
            </label>
          </>
        )}
        ...
        <div className="rounded-lg bg-gray-50 px-3 py-3">
          <p className="text-xs font-bold uppercase text-gray-500">Generated title</p>
          <p className="mt-1 text-sm font-black uppercase text-[#000000]">{getGeneratedChartTitle({ source_type: slot.source_type, exercise_name: slot.exercise_name, metric_key: slot.source_type === 'bodyweight' ? 'bodyweight' : slot.metric_key })}</p>
        </div>

        <button type="button" onClick={onSave} disabled={saving} className="w-full rounded-lg bg-[#FA0201] px-4 py-3 text-sm font-bold uppercase text-white hover:bg-red-700 disabled:opacity-60">
...
export function ClientMetricChartDashboardModalConfig({ clientId }: { clientId: string }) {
  ...
  const loadDashboard = async () => {
    ...
    const hydratedConfigs = [1, 2, 3].map((slot) => loadedConfigs.find((config) => config.slot === slot) || defaults[slot - 1]);
  ...
  const activeEditableSlot = editableSlots.find((slot) => slot.slot === editingSlot) || null;
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <p className="text-sm text-gray-700">Choose up to 3 client-specific performance charts. Configure each chart through its popup.</p>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        {configs.slice(0, 3).map((config) => (
          <ChartCard key={`${config.slot}-${config.metric_key}-${config.exercise_name || 'bodyweight'}`} config={config} points={getChartPoints(config)} onConfigure={() => setEditingSlot(config.slot)} />
        ))}
      </div>

      {activeEditableSlot && (
        <ConfigModal
          slot={activeEditableSlot}
          exerciseNames={exerciseNames}
          hasBodyweightData={hasBodyweightData}
          saving={savingSlot === activeEditableSlot.slot}
          onClose={() => setEditingSlot(null)}
          onChange={(patch) => updateEditableSlot(activeEditableSlot.slot, patch)}
          onSave={() => saveSlot(activeEditableSlot)}
        />
      )}
    </div>
  );
